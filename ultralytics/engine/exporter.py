    ...
    def forward(self, x):
        ...
        for i in range(bs):
            ...
            if self.model.task == "segment":
                detects, extras = out[:bs].split([6, extra_shape], dim=2)
                masks = torch.einsum('b d n, b n h w -> b d h w', extras, preds[1]).sigmoid()
                return detects, masks
            else:
                return out[:bs]
...
class ModelExporter:
    ...
    def load(self, weights: str, version: str, repo_dir: Optional[str] = None,
             custom_classes: Optional[Sequence[str]] = None) -> None:
    ...
    def export(self, output: str, imgsz: Union[int, Sequence[int]], opset_version: int, simplify: bool) -> None:
        ...
        if simplify:
            try:
                ...
                LOGGER.info(f"Simplifying ONNX model with onnxsim version {onnxsim.__version__}...")
                onnx_model, check = onnxsim.simplify(onnx_model)
                assert check, "Simplified ONNX model could not be validated"
            except ImportError:
                LOGGER.warning(
                    'onnxsim not found. Please install onnx-simplifier for example: `pip install onnx-simplifier>=0.4.1`.')
            except Exception as e:
