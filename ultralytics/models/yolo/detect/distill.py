...
class DetectionDistiller(BaseTrainer):
    ...
    def _setup_train(self, world_size):
        ...
        self.model.criterion = self.model.init_criterion(distill=True)
        ...
        if RANK in (-1, 0):
            self.test_loader = self.get_dataloader(self.data.get("val") or self.data.get("test"), batch_size=batch_size * 2, rank=-1, mode='val')
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    self.logical_disloss = (self.logical_disloss * i + log_distill_loss) / (i + 1) if self.logical_disloss is not None else log_distill_loss
                    self.feature_disloss = (self.feature_disloss * i + fea_distill_loss) / (i + 1) if self.feature_disloss is not None else fea_distill_loss
